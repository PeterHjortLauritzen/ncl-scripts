...
setenv all_cases_ncl_files  "(/"\"$data_dir/$cases[1].ave.$avetime.nc.patch_to_nlon{$nlon}xnlat{$nlat}.nc\"","\"$data_dir/$cases[2].ave.$avetime.nc.patch_to_nlon{$nlon}xnlat{$nlat}.nc\"","\"$data_dir/$cases[3].ave.$avetime.nc.patch_to_nlon{$nlon}xnlat{$nlat}.nc\"","\"$data_dir/$cases[4].ave.$avetime.nc.patch_to_nlon{$nlon}xnlat{$nlat}.nc\""/)"
...
#  setenv all_cases_ncl_list  "$all_cases_ncl_list""$data_dir/$interpfile"""\"","\"""
  if (-e $data_dir/$interpfile) then
    ...
    echo "ncl  'lsArg="\"$avefile\""' 'ingrid="\"$case\""' 'outgrid="\"nlon{$nlon}xnlat{$nlat}\""' 'interp_method="\"patch\""' 'grid_dir="\"$work_dir/Regrid_Maps/\""'  'srcPath="\"$data_dir/\""' 'dstPath="\"$work_dir/latlon/\""' 'nx=$nlon' 'ny=$nlat' < $my_ncl_dir/interp_se_to_latlon.ncl" > $work_dir/$case.regrid.sh
  ...
  #
  #*********************************************
foreach var (PS PRECT CLDTOT CLOUD Q T OMEGA U RELHUM V PTEQ PTTEND)
  setenv vname $var
  if (-e $plot_dir/zonal_time_avg_{$vname}.pdf || -e $plot_dir/2d_{$vname}.pdf) then
    ...
    echo "ncl 'vname="\"$vname\""' 'vertical_height = "False"' 'case="$all_cases_ncl_string"' 'lsArg="$all_cases_ncl_files"'  'plot_dir="\"$plot_dir\""' 'plot_lat_section="True"' 'plot_lat_section_min=0' 'plot_lat_section_max=80' 'coslat="False"' 'diff="True"' < $my_ncl_dir/zonal_time_avg.ncl"  > $ncl_script
